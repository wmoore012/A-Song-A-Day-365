  ...
  (
    ...
    const setOpen = React.useCallback(
      (value: boolean | ((value: boolean) => boolean)) => {
        ...
        document.cookie = `${SIDEBAR_COOKIE_NAME}=${encodeURIComponent(String(openState))};`
    ...
    // We check if the sidebar should be collapsed on mount
    React.useEffect(() => {
      const handleResize = () => {
    ...
    // If a sidebar is supposed to be open on mobile, but the window is resized to desktop, we need to close it.
    React.useEffect(() => {
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 translate-x-1/2 cursor-col-resize opacity-0 ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-1 after:-translate-x-1/2 after:rounded-full after:bg-sidebar-border after:opacity-0 after:transition-opacity after:duration-75 hover:after:opacity-100 sm:flex",
